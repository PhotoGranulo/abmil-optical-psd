...
def setup_parser():
    ...
    parser.add_argument("--batch_size", default=16, help="int: batch size", type=int)
...
def save_history(path, history):
    ...
    for key, value in history.history.items():
        # 1. Convert numpy arrays to standard lists
        if isinstance(value, np.ndarray):
            value = value.tolist()
            
        # 2. Force all elements inside the list to be standard Python floats
        if isinstance(value, list):
            new_hist[key] = [float(v) for v in value]
        else:
            new_hist[key] = float(value)
...
def save_dm(file_name, predicted_dm, names):
    """
    Save tile names and predicted dm values to a CSV.
    """
    import numpy as np

    names = np.asarray(names, dtype=str).flatten()
    predicted_dm = np.asarray(predicted_dm, dtype=float).flatten()

    with open(file_name, "w", encoding="utf-8") as f:
        f.write("Name,dm [cm]\n")
        for name, dm in zip(names, predicted_dm):
            f.write(f"{name},{dm:0.6f}\n")
...
def _extract_sid_from_name(name: str):
    m = SID_RX.match(str(name))
    if not m:
        return None
    return int(m.group("sid"))


def wrapper_make_data_split_sample_level(
    experiment_type,
    data_npz_path,
    test_fold_index,
    num_folds=5,
    ...
    """
    Sample-level split for 5-fold CV with no test set. 
    1 fold is used for validation, and the remaining 4 for training.
    The test set is mirrored to the validation set to maintain compatibility with downstream evaluation functions.
    """
    if experiment_type != 'randCV':
        raise ValueError("wrapper_make_data_split_sample_level supports only 'randCV'")

    data_all = np.load(data_npz_path, allow_pickle=True)
    images = data_all["images"]          # (N, H, W, C)
    histograms_cdf = data_all["histograms"]  # (N, 22) CDFs on Lang edges
    dms = data_all["dm"]                # (N,)
    names = data_all["tile_names"]      # (N,)

    # Map each image to a sample ID from its filename prefix
    sids = np.array([_extract_sid_from_name(n) for n in names], dtype=int)
    unique_sids = np.array(sorted(set(sids)))
    np.random.seed(21)
    np.random.shuffle(unique_sids)
    sid_splits = np.array_split(unique_sids, num_folds)

    # 1. Assign the selected fold as the validation set
    val_sids = sid_splits[test_fold_index]
    
    # 2. Assign the remaining folds as the training set
    train_sids = np.concatenate(
        [sid_splits[i] for i in range(num_folds) if i != test_fold_index]
    )

    is_val = np.isin(sids, val_sids)
    is_train = np.isin(sids, train_sids)

    val_indices = np.where(is_val)[0]
    train_indices = np.where(is_train)[0]
    
    # 3. Mirror the test indices to the validation indices for script compatibility
    test_indices = val_indices

    # Convert CDF -> PDF (21 bins) like transform_histogram would
    pdf_all = np.diff(histograms_cdf, axis=1).astype(np.float32)

    fold_data = {}
    fold_data['X_train'] = images[train_indices]
    fold_data['X_val']   = images[val_indices]
    fold_data['X_test']  = images[test_indices]

    if output_dm:
        fold_data['Y_train'] = dms[train_indices]
        fold_data['Y_val']   = dms[val_indices]
        fold_data['Y_test']  = dms[test_indices]
    else:
        fold_data['Y_train'] = pdf_all[train_indices]
        fold_data['Y_val']   = pdf_all[val_indices]
        fold_data['Y_test']  = pdf_all[test_indices]

    fold_data['N_train'] = names[train_indices]
    fold_data['N_val']   = names[val_indices]
    fold_data['N_test']  = names[test_indices]

    fold_data['D_train'] = dms[train_indices]
    fold_data['D_val']   = dms[val_indices]
    fold_data['D_test']  = dms[test_indices]

    # Preprocessing stats (per-channel mean/std on train)
    fold_data['MEAN_train'], fold_data['STD_train'] = prepro.get_mean_and_std_per_channel(
        fold_data['X_train']
    )

    if volume_weighted:
        print('converting labels to relative volume distributions (volume pdf)...')
        fold_data['Y_train'] = relFreq2relVolume(fold_data['Y_train'])
        fold_data['Y_val']   = relFreq2relVolume(fold_data['Y_val'])
        fold_data['Y_test']  = relFreq2relVolume(fold_data['Y_test'])

    if downsample_factor != 1:
        fold_data['X_train'] = downsample_images(fold_data['X_train'], factor=downsample_factor)
        fold_data['X_val']   = downsample_images(fold_data['X_val'],   factor=downsample_factor)
        fold_data['X_test']  = downsample_images(fold_data['X_test'],  factor=downsample_factor)

    return fold_data


def calculate_curve_mae_percent(y_true_pdf, y_pred_pdf):
    """
    Calculates the MAE in percentage between the true and predicted CDF curves.
    """
    # 1. Reconstruct CDFs from PDFs
    # REMOVE "helper." from these two lines:
    cdf_true = cum_histo(y_true_pdf) 
    cdf_pred = cum_histo(y_pred_pdf)

    # 2. Prepend the 0.0 origin to both to get full 22-point curves
    cdf_true_full = np.concatenate(([0.0], cdf_true))
    cdf_pred_full = np.concatenate(([0.0], cdf_pred))

    # 3. Calculate Mean Absolute Error
    mae_fraction = np.mean(np.abs(cdf_true_full - cdf_pred_full))
    
    # 4. Convert to percentage
    return mae_fraction * 100.0