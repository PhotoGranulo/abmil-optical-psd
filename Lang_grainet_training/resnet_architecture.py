...
from tf_keras.layers import (
    add, Conv2D, Activation, MaxPooling2D, AveragePooling2D, 
...
def initialize_weights(model, layer_name=None):
    ...
    Works in TF 2.x / tf_keras without using sessions.
    """
    # In TF2, you normally just call model.build() + model.set_weights, or
    # re-run initializers via backend.reset_uids(), etc.
    # The simplest equivalent here is to use the built-in variable initializer.
    import tensorflow as tf

    if layer_name is None:
        for layer in model.layers:
            if hasattr(layer, 'kernel_initializer'):
                # Recreate kernel variable with the initializer
                weight_shape = layer.kernel.shape
                new_kernel = layer.kernel_initializer(weight_shape, dtype=layer.kernel.dtype)
