...
def mean_IOU(y_true, y_pred):
    ...
    Corrected for compatibility with modern tf-keras.
    """
    # Replace K.tf.where/less_equal with standard backend minimum/maximum
    min_elements = K.minimum(y_true, y_pred)  # element-wise minimum
    max_elements = K.maximum(y_true, y_pred)  # element-wise maximum

    intersection = K.sum(min_elements, axis=-1)
    union = K.sum(max_elements, axis=-1)

    # Use K.epsilon() to safely avoid division by zero
