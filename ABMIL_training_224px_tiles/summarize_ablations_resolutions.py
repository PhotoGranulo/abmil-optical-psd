...
- Computes per-sample metrics (WMAE, KS, WRMSE, Bias).
- Computes per-sieve Median Absolute Error (MAE) for all bins.
- Outputs an Excel file with a single sheet matching the publication table format,
  keeping metrics in decimal form formatted to 4 decimal places.
...
def compute_metrics(pred_cdf: np.ndarray, true_cdf: np.ndarray, bin_x: np.ndarray) -> Dict[str, float]:
    ...
    # Bias as mean signed error per sample
    bias = float(np.sum(signed_diff * weights))

    # Convert log10 bin sizes back to mm for column naming
    bin_mm = np.round(10 ** bin_x, 4)
    bin_errors = {f"MAE_{mm:g}mm": float(d) for mm, d in zip(bin_mm, diff)}

    result = {"ks": ks, "wmae": wmae, "wrmse": wrmse, "bias": bias}
    result.update(bin_errors)
...
def collect_records(root: Path) -> pd.DataFrame:
    ...
    for preds_path in iter_pred_files(root):
        ...
        for rec in pred_list:
            ...
            # Append the per-sieve errors automatically
...
def build_master_table(df: pd.DataFrame, metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    def make_condition(row) -> str:
        ...
        # Remove the T224 prefix to keep the table cell clean
        row_id = row.get("row_id", "").replace("T224/", "")
        
        # Clean up resolution ablations specifically if present
        row_id = row_id.replace("T224_25", "25%").replace("T224_50", "50%").replace("T224_75", "75%").replace("T224", "100%")
        
        # If it's just the basic sample count sweep, keep the condition simple (e.g. N_128)
        if scenario == "train_pool_sweep":
            return row_id
        
        # Otherwise, combine the scenario name and parameters so different ablations don't mix
        if row_id:
    ...
    # Helper function to ensure N_32 sorts before N_128, and Tray1 sorts before Tray3
    ...
    # Find all MAE columns dynamically
    mae_cols = [c for c in df_merged.columns if c.startswith("MAE_")]

    # Average identical samples over folds within the same run 
    cols_to_mean = ["wmae", "ks", "wrmse", "bias"] + mae_cols
    df_run_sid = df_merged.groupby(["Condition", "_sort_tup", "run_id", "sid", "size_group"], as_index=False)[cols_to_mean].mean()
    ...
    # Sort MAE columns by descending sieve size (e.g., 80mm before 56mm before 40mm)
    def extract_mm(col_name: str) -> float:
        match = re.search(r"MAE_([\d\.]+)mm", col_name)
        return float(match.group(1)) if match else 0.0
        
    sorted_mae_cols = sorted(mae_cols, key=extract_mm, reverse=True)
    ...
    for size_filter, label in groups_to_eval:
        ...
        for cond_val, n_df in group_df.groupby("Condition"):
            ...
            # 1. Compute median (or mean for bias) and 90th percentile across samples for EACH run independently
            cols_for_median = ["wmae", "ks", "wrmse"] + sorted_mae_cols
            run_medians = n_df.groupby("run_id")[cols_for_median].median()
            run_means = n_df.groupby("run_id")[["bias"]].mean() # Using mean for signed error bias
            ...
            res_dict = {
                ...
                "Bias": float(final_means["bias"]),
                "_sort_tup": sort_tup # Hidden column for sorting
            }
            
            # Add all the Per-Sieve Medians (Sorted from largest to smallest mm)
            for col in sorted_mae_cols:
                clean_name = col.replace("_", " ") # Converts "MAE_80mm" to "MAE 80mm"
                res_dict[clean_name] = float(final_medians[col])
    ...
    if not df_res.empty:
        ...
        # Sort by Type, then use our hidden tuple to ensure mathematical sorting of the parameters
        df_res = df_res.sort_values(["Type", "_sort_tup"]).reset_index(drop=True)
        df_res = df_res.drop(columns=["_sort_tup"])
        
        # Visual trick: Blank out duplicated "Type" names so it looks like the paper draft
...
def write_excel_clean(df: pd.DataFrame, out_path: Path) -> None:
    ...
    if ws.max_row > 1:
        ...
        for name, col_idx in col_map.items():
            ...
            # Formatting rules based on column names
            if "Validation" in name or "samples" in name:
                for r in range(2, ws.max_row + 1):
                    ws.cell(row=r, column=col_idx).number_format = "0"
            elif any(metric in name for metric in ["WMAE", "KS", "WRMSE", "Bias", "MAE"]):
                # Set format to display 4 decimal places in Excel (e.g., 0.0564)
                for r in range(2, ws.max_row + 1):
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    # Build exact table
    master_table = build_master_table(df, metadata)
