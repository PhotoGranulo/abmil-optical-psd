...
ROW_SETUPS: Dict[str, RowSetup] = {
    ...
    "T224": RowSetup("T224", (Path("/home/thomas_plante_stcyr/workspace/torch/2_1_1/scratch/dinov2_features_7616x5440/dinov2_224_grid_34x24"),), (24, 34), 224),

    # Resolution setups for the ablation (100% already above)
    "T224_75":  RowSetup("T224_75",  (Path("/home/thomas_plante_stcyr/workspace/torch/2_1_1/scratch/dinov2_224_res75_50_25/Res075"),), (18, 25), 224),
    "T224_50":  RowSetup("T224_50",  (Path("/home/thomas_plante_stcyr/workspace/torch/2_1_1/scratch/dinov2_224_res75_50_25/Res050"),), (12, 17), 224),
    "T224_25":  RowSetup("T224_25",  (Path("/home/thomas_plante_stcyr/workspace/torch/2_1_1/scratch/dinov2_224_res75_50_25/Res025"),), (6, 8), 224),
    "T224_10":  RowSetup("T224_10",  (Path("/home/thomas_plante_stcyr/workspace/torch/2_1_1/scratch/dinov2_224_res75_50_25/Res010"),), (6, 8), 224),
}
...
STUDY_PLANS: Dict[str, StudyPlan] = {
    ...
    "feature_composition": StudyPlan(
        name="feature_composition",
        description="Ablate tile-level features and global bag embedding.",
        scenarios=(
            Scenario(key="cls_only_no_global", description="Only CLS vector, no global embedding in bag.", cfg_overrides={"FEATURE_MODE": "cls_only", "ADD_GLOBAL_TILE": False}),
            Scenario(key="cls_only_with_global", description="Only CLS vector, with global embedding in bag.", cfg_overrides={"FEATURE_MODE": "cls_only", "ADD_GLOBAL_TILE": True}),
            Scenario(key="cls_global_no_global", description="CLS + Global broadcast, no global embedding in bag.", cfg_overrides={"FEATURE_MODE": "cls_global", "ADD_GLOBAL_TILE": False}),
            Scenario(key="cls_global_with_global", description="CLS + Global broadcast, with global embedding in bag.", cfg_overrides={"FEATURE_MODE": "cls_global", "ADD_GLOBAL_TILE": True}),
            Scenario(key="full_no_global", description="Current setup (CLS+Mean+Global+Pos), no global embedding in bag.", cfg_overrides={"FEATURE_MODE": "full", "ADD_GLOBAL_TILE": False}),
            Scenario(key="full_with_global", description="Current setup (CLS+Mean+Global+Pos), with global embedding in bag.", cfg_overrides={"FEATURE_MODE": "full", "ADD_GLOBAL_TILE": True}),
        ),
        requires_row_choice=True,
        default_row_id="T224",
    ),
...
class BagDataset(torch.utils.data.Dataset):
    def __init__(self, sids, by_sample, psd_map, k_imgs=4, seed=0, pos_fourier=None,
                 ...
                 feature_mode: str = "full"):  # ADDED
        self.sids = list(sids)

        self.feature_mode = str(feature_mode).lower()
        if self.feature_mode not in {"full", "cls_only", "cls_global"}:
            raise ValueError(f"Unknown feature_mode={feature_mode!r}. Choose from: full, cls_only, cls_global")
    ...
    def _tile_features(self, d) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
        ...
        if self.feature_mode == "cls_only":
            X = cls
        elif self.feature_mode == "cls_global":
            X = np.concatenate([cls, g_broadcast], axis=1)
        else:  # "full"
    ...
    def _build_bag_from_captures(self, sid: int, sel: List[str], extra_vec: Optional[np.ndarray]) -> Dict[str, object]:
        ...
        for img_i, path in enumerate(sel):
            ...
            if self.add_global_tile:
                # MODIFIED: global tile must match current feature_mode dimensionality
                if self.feature_mode == "cls_only":
                    g_tile = g_cap[None, :]
                elif self.feature_mode == "cls_global":
                    g_tile = np.concatenate([g_cap, g_cap], axis=0)[None, :]
                else:  # "full"
                    pad = int(X_local.shape[1] - 3 * g_cap.size)
                    if pad < 0:
                        raise RuntimeError(
                            f"Global tile padding underflow: X_local_dim={X_local.shape[1]} vs 3*g_dim={3*g_cap.size}"
                        )
                    zeros_pos = np.zeros((pad,), dtype=np.float32)
                    g_tile = np.concatenate([g_cap, g_cap, g_cap, zeros_pos], axis=0)[None, :]
...
def infer_dims(sample_ids: List[int], by_sample, psd_map, bag_k: int, seed: int, add_global_tile: bool, 
               moisture_filter: Optional[Collection[str]] = None, sample_features: Optional[Dict[int, np.ndarray]] = None,
               feature_mode: str = "full") -> Tuple[int, int]:  # ADDED
    ds = BagDataset(sample_ids[:1], by_sample, psd_map, k_imgs=bag_k, seed=seed,
                    add_global_tile=add_global_tile, moisture_filter=moisture_filter, sample_features=sample_features,
                    feature_mode=feature_mode)  # ADDED
    b = ds[0]
...
def _run_scenario_for_row(plan: StudyPlan, scenario: Scenario, row: RowSetup, row_id: str, psd_map: Dict[int, np.ndarray],
    ...
    feature_mode = cfg_overrides.get("FEATURE_MODE", "full")
    ...
    "row_id": row_id,
    "tile_px": row.px,
    "tiles_hw": row.tiles_hw,
    "cfg_overrides": dict(cfg_overrides),
    "add_global_tile": add_global_tile,
    "feature_mode": feature_mode,  # ADDED
    "loss_key": loss_key,
    "sample_count": len(sample_ids),
    "train_sizes": list(N_list),
    "k_folds": CFG["K_FOLDS"],
    "repeats": repeats,
    "timestamp": stamp,
    }
    ...
    _run_training_for_row(plan=plan, scenario=scenario, row=row, row_id=row_id, sample_ids=sample_ids, 
                          ...
                          BIN_X=BIN_X, psd_map=psd_map, scenario_root=scenario_root, repeats=repeats, base_seed=base_seed, by_sample=by_sample,feature_mode=feature_mode,)
...
def _run_training_for_row(plan: StudyPlan, scenario: Scenario, row: RowSetup, row_id: str, sample_ids: Sequence[int], 
                          ...
                          repeats: int, base_seed: int, by_sample: Dict[int, List[str]],feature_mode: str = "full") -> None:
    ...
    d_in, n_bins = infer_dims(sample_ids, by_sample=by_sample, psd_map=psd_map, bag_k=CFG["BAG_K"], seed=base_seed,
                              add_global_tile=add_global_tile, moisture_filter=moisture_filter, 
                              sample_features=scenario_sample_features,feature_mode=feature_mode)
        ...
        for N in N_list:
            for r in range(repeats):
                ...
                for fold in range(CFG["K_FOLDS"]):
                    ...
                    tr_loader = make_loader(tr_ids, by_sample, psd_map, CFG["BATCH_SIZE"], True, CFG["NUM_WORKERS"], train_bag_k, rep_seed, add_global_tile=add_global_tile, 
                                            images_per_tray=train_images_per_tray if expected_tray_count is not None else None, expected_tray_count=expected_tray_count, 
                                            shortage_tracker=train_tracker, phase="train", regime=scenario.filter_key, context={"N": N, "repeat": r, "combo_label": combo_label}, 
                                            moisture_filter=moisture_filter, sample_features=scenario_sample_features, feature_mode=feature_mode)
                    va_loader = make_loader(va_ids, by_sample, psd_map, CFG["BATCH_SIZE"], False, CFG["VAL_WORKERS"], val_bag_k, rep_seed, add_global_tile=add_global_tile, 
                                            images_per_tray=val_images_per_tray if expected_tray_count is not None else None, expected_tray_count=expected_tray_count, 
                                            shortage_tracker=val_tracker, phase="val", regime=scenario.filter_key, context={"N": N, "repeat": r, "combo_label": combo_label}, 
                                            moisture_filter=moisture_filter, sample_features=scenario_sample_features, feature_mode=feature_mode)
